...
void SDQ::norm(map<int, double> & P){
    //tested
...
void SDQ::Subsampling(vector<vector<double>>& img){
    ...
    int hidx;
    int vidx;
    for(i=0, hidx=0; i<pad_rows; i+=SDQ::SmplVstep, hidx+=1){
        for(j=0, vidx=0;j<pad_cols;j+=SDQ::SmplHstep,vidx+=1){
            if(j<Ncols && i<Nrows){
...
void SDQ::Upsampling(vector<vector<double>>& img){
    ...
    int x,y,xidx,yidx;
    int i,j;
    for(i=Nsampled_row-1;i>-1;i-=1){
        for(j=Nsampled_col-1;j>-1;j-=1){
            for(x=SDQ::SmplHstep-1; x>-1; x-=1){
                for(y=SDQ::SmplVstep-1; y>-1; y-=1){
