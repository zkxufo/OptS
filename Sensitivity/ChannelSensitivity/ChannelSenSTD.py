...
def main(model = 'alexnet', Batch_size = 100, Nexample= 10000, grace=True):
    ...
    for data, target in tqdm(test_loader):
        ...
        ycbcr_data = rgb_to_ycbcr(data,
                                  W_r=A[0],
                                  W_g=A[1],
                                  W_b=A[2]).transpose(1, 0).reshape(3, -1)
        ycbcr_data.requires_grad = True
        ycbcr_IMG = ycbcr_data.reshape(3, Batch_size, 224, 224).transpose(1, 0)
        recovered_img = ycbcr_to_rgb(ycbcr_IMG,
                                     W_r=A[0],
                                     W_g=A[1],
                                     W_b=A[2])
        norm_img1 = Scale2One(recovered_img)  # [0,1]
